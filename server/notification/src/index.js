...
const send = async (title, action) => {
  if (action == "create")
    io.emit("notification", { message: `New todo created: ${title}` })
  if (action == "update_marked")
    io.emit("notification", { message: `Todo marked as complete: ${title}` })
  if (action == "update_unmarked")
    io.emit("notification", { message: `Todo marked as incomplete: ${title}` })

  if (action == "delete")
    io.emit("notification", { message: `Todo deleted: ${title}` })
}
